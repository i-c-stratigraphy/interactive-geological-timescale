...
export default {
                ...
                if (xmlHttp.readyState == 4 && xmlHttp.status == 200){
                    ...
                    if (dataType == 'jsonElementData'){
                        ...
                        if (this_.edgeData.result.primaryTopic.stratotype != null && !(this_.edgeData.result.primaryTopic.stratotype instanceof Array)){
                            this_.httpRequestAsync('http://vocabs.ardc.edu.au/repository/api/lda/csiro/international-chronostratigraphic-chart/geologic-time-scale-2020/resource.json?uri=' + this_.edgeData.result.primaryTopic.stratotype._about, 'stratotypeData')
                            ...
                            for (var stratotype in this_.edgeData.result.primaryTopic.stratotype){
                                this_.httpRequestAsync('http://vocabs.ardc.edu.au/repository/api/lda/csiro/international-chronostratigraphic-chart/geologic-time-scale-2020/resource.json?uri=' + this_.edgeData.result.primaryTopic.stratotype[stratotype]._about, 'stratotypeData')
                            }
                        }
        ...
        goToPrevious: function() {
            if (this.edgeDataReceived){
                ...
                if (this.history.length == 1) {
                    ...
                    var requestURL = 'http://vocabs.ardc.edu.au/repository/api/lda/csiro/international-chronostratigraphic-chart/geologic-time-scale-2020/resource.json?uri=' + url
                    this.$emit('input', url)
    ...
    mounted(){
        var requestURL = 'http://vocabs.ardc.edu.au/repository/api/lda/csiro/international-chronostratigraphic-chart/geologic-time-scale-2020/resource.json?uri=' + this.id
        this.httpRequestAsync(requestURL, 'jsonElementData')
        EventBus.$on('get-more-data', url => {
            this.history.push([this.id, this.jsonElementData.result.primaryTopic.label._value])
            this.httpRequestAsync('http://vocabs.ardc.edu.au/repository/api/lda/csiro/international-chronostratigraphic-chart/geologic-time-scale-2020/resource.json?uri=' + url, 'edgeData')
        })
        ...
        EventBus.$on('update-data', node =>{
            this.dataReceived = false
            requestURL = 'http://vocabs.ardc.edu.au/repository/api/lda/csiro/international-chronostratigraphic-chart/geologic-time-scale-2020/resource.json?uri=' + node[0]
